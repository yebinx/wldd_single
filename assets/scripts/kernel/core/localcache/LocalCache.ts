...
export default class LocalCache
{
	...
	{
		this._name = name;
		this._localStorage = sys.localStorage;
		this._dataMap = {};
	}
	...
	public read(key:string, defaultV?:any) : any
	{
		...
		logger.log("读取：", this.localKey(key), key, typeof value, value);
		return value;
	}

	//更新缓存
	public write(key:string, value:string|number) : void
	{
		if(this._dataMap[key]===value) { 
			logger.log("无需存储：", this.localKey(key), key, typeof value);
			return; 
		}
		this._dataMap[key] = value;
		logger.log("存储：", this.localKey(key), key, typeof value);
